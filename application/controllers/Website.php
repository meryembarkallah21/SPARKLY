...
class Website extends CI_Controller {
	...
	public function item($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('logged_in') == TRUE){
			$where_admin['admin_user'] 	= $this->session->userdata('admin_user');
			$data['admin'] 				= $this->ADM->get_admin('',$where_admin);
			...
			$data['content'] 			= 'admin/content/website/item';
			$data['menu_terpilih']		= '1';
			$data['submenu_terpilih']	= '13';
			$data['action']				= (empty($filter1))?'view':$filter1;
			$data['validate']			= array('nama_item'=>'Name',
												'merek'=>'Brand'
											);
			if ($data['action'] == 'view'){
				$data['berdasarkan']		= array('nama_item'=>'NAME','merek'=>'BRAND');
				$data['cari']				= ($this->input->post('cari'))?$this->input->post('cari'):'nama_item';
				$data['q']					= ($this->input->post('q'))?$this->input->post('q'):'';
				$data['halaman']			= (empty($filter2))?1:$filter2;
				$data['batas']				= 10;
				$data['page']				= ($data['halaman']-1) * $data['batas'];
				$like_item[$data['cari']]	= $data['q'];
				$data['jml_data']			= $this->ADM->count_all_item('', $like_item);
				$data['jml_halaman'] 		= ceil($data['jml_data']/$data['batas']);
			} elseif ($data['action'] == 'tambah'){
			if ($data['admin']->admin_level_kode == 1 || $data['admin']->admin_level_kode == 2) {
				$data['onload']				= 'item';
				$data['nama_item']	= ($this->input->post('nama_item'))?$this->input->post('nama_item'):'';
				$data['merek']	= ($this->input->post('merek'))?$this->input->post('merek'):'';
				$simpan						= $this->input->post('simpan');
				if ($simpan){
					$insert['nama_item']			= validasi_sql($data['nama_item']);
					$insert['merek']			= validasi_sql($data['merek']);
					$insert['stock']			= 0;
					$this->ADM->insert_item($insert);
				...
				if ($data['admin']->admin_level_kode == 1 || $data['admin']->admin_level_kode == 2) {
				$data['onload']				= 'nama_item';
				$where_item['id_item']	= $filter2; 
				$item				= $this->ADM->get_item('*', $where_item);
				$data['id_item']	= ($this->input->post('id_item'))?$this->input->post('id_item'):$item->id_item;				
				$data['nama_item']	= ($this->input->post('nama_item'))?$this->input->post('nama_item'):$item->nama_item;				
				$data['merek']	= ($this->input->post('merek'))?$this->input->post('merek'):$item->merek;				
				$simpan						= $this->input->post('simpan');
				if ($simpan){
					$where_edit['id_item']	= $data['id_item'];
					$edit['nama_item']	= $data['nama_item'];
					$edit['merek']	= $data['merek'];
					$this->ADM->update_item($where_edit, $edit);
				...
				if ($data['admin']->admin_level_kode == 1 || $data['admin']->admin_level_kode == 2) {
				$where_delete['id_item']		= validasi_sql($filter2);
				$this->ADM->delete_item($where_delete);
	...
	public function masuk($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('logged_in') == TRUE){
			...
			$data['submenu_terpilih']	= '13';
			$data['action']				= (empty($filter1))?'view':$filter1;
			$data['validate']			= array('id_item'=>'Item name'
											);
			if ($data['action'] == 'view'){
				$data['berdasarkan']		= array('id_item'=>'ITEM NAME');
				$data['cari']				= ($this->input->post('cari'))?$this->input->post('cari'):'id_item';
				$data['q']					= ($this->input->post('q'))?$this->input->post('q'):'';
				$data['halaman']			= (empty($filter2))?1:$filter2;
				$data['batas']				= 10;
				$data['page']				= ($data['halaman']-1) * $data['batas'];
			...
			if ($data['admin']->admin_level_kode == 1 || $data['admin']->admin_level_kode == 2) {
				$data['onload']				= 'supplier';
				$data['id_item']	= ($this->input->post('id_item'))?$this->input->post('id_item'):'';
				...
				$simpan						= $this->input->post('simpan');
				if ($simpan){
					$insert['id_item']			= validasi_sql($data['id_item']);
					...
					$where_item['id_item']	= $data['id_item']; 
					$item	= $this->ADM->get_item('*', $where_item);

					$where_edit['id_item']	= $data['id_item'];
					$edit['stock']	= $item->stock +$data['jumlah'] ;
					$this->ADM->update_item($where_edit, $edit);
...
if ($item->stock <= $limitstock->stock) {
					$message = "Stock with products ".$item->nama_item." less than the minimum stock limit";
					$user_id = 4444;
					$url = "";
					$headings = "Stock Warning";
					$img = "/assets/bg.svg";
