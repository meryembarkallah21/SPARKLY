...
class M_admin extends CI_Model  {
    ...
    public function update_item($where,$data){
        $this->db->update("master_item",$data,$where);
    }

    public function delete_item($where){
        $this->db->delete("master_item", $where);
    }

	public function get_item($select, $where){
        $data = "";
		$this->db->select($select);
    ...
    public function grid_all_item($select, $sidx, $sord, $limit, $start, $where="", $like=""){
        $data = "";
        $this->db->select($select);
        $this->db->from("master_item");
		if ($where){$this->db->where($where);}
    ...
    public function count_all_item($where="", $like=""){
        $this->db->select("*");
        $this->db->from("master_item");
		if ($where){$this->db->where($where);}
		if ($like){
...
//CONFIGURATION TABEL LIMIT STOCK
    ...
    public function update_transaksi($where,$data){
        $this->db->update("transaksi_item",$data,$where);
    }

    public function delete_transaksi($where){
        $this->db->delete("transaksi_item", $where);
    }
	...
	public function get_low_stock(){
		...
		$sql = "SELECT *,(COALESCE((SELECT SUM(jumlah) FROM transaksi_item where id_item = master_item.id_item and status_pergerakan = 1), 0) - COALESCE((SELECT SUM(jumlah) FROM transaksi_item where id_item = master_item.id_item and status_pergerakan = 2), 0)) as `qty` FROM master_item where (COALESCE((SELECT SUM(jumlah) FROM transaksi_item where id_item = master_item.id_item and status_pergerakan = 1), 0) - COALESCE((SELECT SUM(jumlah) FROM transaksi_item where id_item = master_item.id_item and status_pergerakan = 2), 0)) <= $limit_stock order by nama_item asc";
